...
input ENUM_LINE_STYLE MedianStyle             = STYLE_DOT;
input ENUM_LINE_STYLE MedianRayStyle          = STYLE_DOT;
input ENUM_LINE_STYLE ValueAreaSidesStyle     = STYLE_SOLID;
input ENUM_LINE_STYLE ValueAreaHighLowStyle   = STYLE_DOT;
...
input bool           HideRaysFromInvisibleSessions = true;     // HideRaysFromInvisibleSessions: hide rays from behind the screen.
input int            TimeShiftMinutes         = 0;              // TimeShiftMinutes: shift session + to the left, - to the right.
input bool           ShowKeyValues            = false;           // ShowKeyValues: print out VAH, VAL, POC on chart.
...
input bool           DisableHistogram         = true;  // DisableHistogram: do not draw profile, VAH, VAL, and POC still visible.
...
input bool           EnableIntradaySession1      = false;
...
input bool           EnableIntradaySession2      = false;
...
input bool           EnableIntradaySession3      = false;
...
input bool           EnableIntradaySession4      = false;
...
double SessionMedianBuffer[]; // Buffer for session median price
...
int OnInit()
{
    ...
    // Initialization for Session Median Buffer (index 6)
    SetIndexBuffer(6, SessionMedianBuffer, INDICATOR_DATA);
    PlotIndexSetInteger(6, PLOT_DRAW_TYPE, DRAW_NONE);   // Draw as a line (can be DRAW_NONE if not for direct plotting)
    PlotIndexSetDouble(6, PLOT_EMPTY_VALUE, EMPTY_VALUE); // Value to use for empty/non-calculated points
    ArraySetAsSeries(SessionMedianBuffer, true);          // Important for correct indexing
               ...
               )
{
    ...
    if (((EnableDevelopingPOC) || (EnableDevelopingVAHVAL)) && (rates_total - prev_calculated > 1) && (CleanedUpOn != rates_total))
    {
        ...
        for (int i = prev_calculated; i < rates_total; i++)
        {
            ...
            SessionMedianBuffer[i] = EMPTY_VALUE;
        }
...
bool ProcessSession(const int sessionstart, const int sessionend, const int i, const double& High[], const double& Low[], const datetime& Time[], const int rates_total, CRectangleMP* rectangle = NULL)
{
    ...
    // ADD THE NEW BUFFER POPULATION CODE HERE:
    if (PriceOfMaxRange != 0 && PriceOfMaxRange != EMPTY_VALUE) // Ensure POC was calculated
    {
        for (int j = sessionstart; j >= sessionend; j--)
        {
            // Ensure we don't write out of bounds if sessionstart/end are near array limits
            if (j < rates_total && j >= 0) 
            {
                SessionMedianBuffer[j] = PriceOfMaxRange;
            }
        }
    }
    else // If POC is not valid, fill session with EMPTY_VALUE
    {
        for (int j = sessionstart; j >= sessionend; j--)
        {
            if (j < rates_total && j >= 0)
            {
                SessionMedianBuffer[j] = EMPTY_VALUE;
            }
        }
    }
