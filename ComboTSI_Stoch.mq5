...
input enColorOn          inpColorOn      = col_onSignalCross; // TSI mode
...
input enChartStyle inpChartStyle = STYLE_COLOR_CANDLES; // Price visualization style
...
double signalValueBuffer[]; // NEW BUFFER: For EA consumption (same values as colorIndex)
                ...
                const int &spread[])
{
   ...
   if(start == 1)
   {
      double currentPrice = GetPrice(inpPrice, open[0], high[0], low[0], close[0]);
      TSI[0] = 0;
      TSI_signal[0] = 0;
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      // Calculate EMAs
      double ema1 = (i == 1) ? momentum : TSI[i-1] + alpha1 * (momentum - TSI[i-1]);
      double emaabs1 = (i == 1) ? fabs(momentum) : fabs(TSI[i-1]) + alpha1 * (fabs(momentum) - fabs(TSI[i-1]));
      double ema2 = (i == 1) ? ema1 : TSI_signal[i-1] + alpha2 * (ema1 - TSI_signal[i-1]);
      double emaabs2 = (i == 1) ? emaabs1 : fabs(TSI_signal[i-1]) + alpha2 * (emaabs1 - fabs(TSI_signal[i-1]));
