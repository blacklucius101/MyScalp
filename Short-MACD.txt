   ...
   for(int bar=limit; bar>=0 && !IsStopped(); bar--)
     {
      ...
      if(LineBuffer[bar]>0)
        {
         if(LineBuffer[bar]>LineBuffer[bar+1]) clr=4; // green - bullish momentum increasing
         if(LineBuffer[bar]<LineBuffer[bar+1]) clr=3; // orange - bullish momentum decreasing
        }
      if(LineBuffer[bar]<0)
        {
         if(LineBuffer[bar]<LineBuffer[bar+1]) clr=0; // red - bearish momentum increasing
         if(LineBuffer[bar]>LineBuffer[bar+1]) clr=1; // lime - bearish momentum decreasing
        }
      ColorBuffer[bar]=clr;
     }
